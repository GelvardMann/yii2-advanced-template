...
class VerifyEmailFormTest extends \Codeception\Test\Unit
{
    ...
    public function testVerifyWrongToken()
    {
        $this->tester->expectException('\yii\base\InvalidArgumentException', function() {
            new \common\modules\user\forms\VerifyEmailForm('');
        });

        $this->tester->expectException('\yii\base\InvalidArgumentException', function() {
            new \common\modules\user\forms\VerifyEmailForm('notexistingtoken_1391882543');
        });
    }
    ...
    {
        $this->tester->expectException('\yii\base\InvalidArgumentException', function() {
            new \common\modules\user\forms\VerifyEmailForm('already_used_token_1548675330');
        });
    }

    public function testVerifyCorrectToken()
    {
        $model = new \common\modules\user\forms\VerifyEmailForm('4ch0qbfhvWwkcuWqjN8SWRq72SOw1KYT_1548675330');
